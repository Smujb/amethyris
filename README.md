...
The deafult terminal is kitty, but I may end up changing this to foot as it is a more basic default. Most terminals work fine as an AppImage or even a flatpak, and some also offer support for a local installation in `~/.local/share`.
...
Likely never. Arch's rolling nature makes this incredibly annoying to do; this would require us to switch over to building each image with the latest archive for one to stop links from 404ing. That could cause issues with third-party repositories. Partial updates from using `-Sy` and layered system updates with `-Syu` are not reasonable to support either.
